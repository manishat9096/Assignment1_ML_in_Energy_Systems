...
def samples_generation(T, n_samples):
    windfarm_capacity = pd.read_csv(
        "Datasets/Capacity of wind farms.csv", delimiter=";"
    )
    ...
    for wf_index, wf_name in enumerate(windfarm_samples.keys()):
        ...
        for i in range(n_samples):
            ...
            # Simulate a wind production pattern
            diurnal_pattern = 0.5 * max_capacity * (
                1 + np.sin(2 * np.pi * (hours - np.random.uniform(0, 12)) / 24)
            ) + np.random.normal(
                0, 0.2 * max_capacity, T
            )  # Larger noise component
            random_dip = np.exp(
                -((hours - np.random.uniform(6, 18)) ** 2) / 10
            )  # Random drop in production
            diurnal_pattern *= 1 - 0.3 * random_dip  # Introduce temporary dips
            diurnal_pattern = np.clip(
                diurnal_pattern, 0, max_capacity
            )  # Ensure within capacity

            windfarm_samples[wf_name][i].extend(diurnal_pattern.tolist())
    ...
    for load_index, load_name in enumerate(loads_samples.keys()):
        ...
        for i in range(n_samples):
            ...
            # Simulate a demand profile with peaks in morning and evening
            morning_peak = np.exp(-((hours - 8) ** 2) / 10)  # Morning peak around 8 AM
            evening_peak = np.exp(-((hours - 18) ** 2) / 10)  # Evening peak around 6 PM
            baseline = 0.3 * max_load  # Baseline demand
            daily_pattern = baseline + max_load * 0.5 * (morning_peak + evening_peak)
            daily_pattern += np.random.normal(0, 0.1 * max_load, T)  # Add some noise

            daily_pattern = np.clip(
                daily_pattern, 0, max_load
            )  # Ensure within max load

            loads_samples[load_name][i].extend(daily_pattern.tolist())

    return windfarm_samples, loads_samples


def plot_samples(windfarm_samples, load_samples, T, n_samples):
    hours = np.arange(0, T)
    scenario_colors = plt.cm.tab10(
        np.linspace(0, 1, n_samples)
    )  # Use a colormap with distinct colors

    # Plot wind farm production
    plt.figure(figsize=(12, 8))
    count = 0
    for samples in windfarm_samples:
        if count < n_samples:
            plt.plot(hours, samples)
    plt.title("Wind Farm Production Over 24 Hours (10 Samples)")
    ...
    plt.figure(figsize=(12, 8))
    count = 0
    for samples in load_samples[:n_samples]:
        if count < n_samples:
            plt.plot(hours, samples)
    plt.title("Load Demand Over 24 Hours (10 Samples)")
    plt.xlabel("Hour of the Day")
    plt.ylabel("Load (MW)")
    plt.grid()
    plt.show()
...
def opti_schedule(T, windfarm_samples, loads_samples, sample_n):
    ...
    data_generators = [(1, "G1"), (2, "G2"), (6, "G3"), (4, "WF1"), (5, "WF2")]
    ...
    # Import data
    B = pd.read_csv(
        "B (power transfer factor of each bus to each line).csv", delimiter=";"
    )
    lines_capacity = pd.read_csv("Transmission capacity of lines.csv", delimiter=";")

    P_max_data = pd.read_csv(
        "Maximum production of generating units.csv", delimiter=";"
    )
    P_min_data = pd.read_csv(
        "Minimum production of generating units.csv", delimiter=";"
    )
    R_rate_data = pd.read_csv("Ramping rate of generating units.csv", delimiter=";")
    DT_min_data = pd.read_csv(
        "Minimum down time of generating units.csv", delimiter=";"
    )
    ...
    start_cost_data = pd.read_csv(
        "Start-up cost of generating units.csv", delimiter=";"
    )
    ...
    # Line definiton
    lines_def = [(1, 2), (2, 3), (1, 4), (2, 4), (4, 5), (5, 6), (3, 6)]

    for line in range(len(lines_def)):
        lines_def[line] = tuple(list(lines_def[line]) + [lines_capacity["fmax"][line]])
    ...
    generator_production = {
        (g, h): model.addVar(name=f"Electricity production of generator {g} t={h}")
        for g in GENERATORS
        for h in range(T)
    }

    generator_start_cost = {
        (g, h): model.addVar(lb=0, name=f"Starting cost of generator {g} t={h}")
        for g in GENERATORS
        for h in range(T)
    }
    ...
    generator_commitment = {
        (g, h): model.addVar(
            vtype=gb.GRB.BINARY, name=f"Committing generator {g} t={h}"
        )
        for g in GENERATORS
        for h in range(T)
    }
    ...
    # Slack variables
    eta = {
        (n, h): model.addVar(lb=0, name=f"Slack variable eta {n} t={h}")
        for n in BUSES
        for h in range(T)
    }

    delta = {
        (n, h): model.addVar(lb=0, name=f"Slack variable delta {n} t={h}")
        for n in BUSES
        for h in range(T)
    }

    # Set objective function
    ED_objective = gb.quicksum(
        P_cost[g] * generator_production[(g, h)] + generator_start_cost[(g, h)]
        for g in GENERATORS
        for h in range(T)
    ) + M * (gb.quicksum(eta[(n, h)] + delta[(n, h)] for n in BUSES for h in range(T)))
    ...
    for h in range(T):
        ...
        balance_constraint = model.addConstr(
            sum(windfarm_samples[wf][sample_n][h] for wf in WINDFARMS)
            + gb.quicksum(generator_production[(g, h)] for g in GENERATORS),
            gb.GRB.EQUAL,
            sum(loads_samples[load][sample_n][h] for load in LOADS)
            + gb.quicksum(eta[(n, h)] - delta[(n, h)] for n in BUSES),
        ...
        for l, (node1, node2, limit) in enumerate(lines_def):
            ...
            constraint_lb = model.addConstr(
                ...
                gb.quicksum(
                    ...
                    * (
                        sum(
                            windfarm_samples[wf][sample_n][h]
                            for wf in WINDFARMS
                            if generator_nodes.get(wf) == node_i
                        )
                        + gb.quicksum(
                            generator_production[(g, h)]
                            for g in GENERATORS
                            if generator_nodes.get(g) == node_i
                        )
                        - sum(
                            loads_samples[load_][sample_n][h]
                            for load_ in LOADS
                            if load_nodes.get(load_) == node_i
                        )
                        - gb.quicksum(eta[(n, h)] for n in BUSES if n == f"Bus{node_i}")
                        + gb.quicksum(
                            delta[(n, h)] for n in BUSES if n == f"Bus{node_i}"
                        )
                    )
        ...
        if h > 0:
            start_up_constraint = {
                model.addConstr(
                    start_cost[g]
                    * (generator_commitment[(g, h)] - generator_commitment[(g, h - 1)]),
                    gb.GRB.LESS_EQUAL,
            ...
            for g in GENERATORS:
                ...
                minimum_DT_constr = {
                    model.addConstr(
                        -generator_commitment[(g, h - 1)]
                        + generator_commitment[(g, h)]
                        - generator_commitment[(g, tau)],
                        gb.GRB.LESS_EQUAL,
                        0,
    ...
    # Plotting Results
    production_data = {
        g: [generator_production[(g, h)].X for h in range(T)] for g in GENERATORS
    }
    unit_commitment = {
        g: [generator_commitment[(g, h)].X for h in range(T)] for g in GENERATORS
    }
    ...
    fig, axes = plt.subplots(
        len(unit_commitment),
        1,
        figsize=(16, 3 * len(unit_commitment)),
        sharex=True,
        dpi=300,
    )
...
plot_samples(windfarm_samples["WF1"], loads_samples["L1"], T, 15)

    ...
    for h in range(T):
        ...
        constr_gen_max = {
            model.addConstr(
                generator_production[(g,h)],
                gb.GRB.LESS_EQUAL,
                generator_commitment[(g,h)] * P_max[g],              # ERROR, should it be prd*com <= Pmax ?
        ...
        for l, (node1, node2, limit) in enumerate(lines_def):
            for node_i in range(1, len(BUSES) + 1):
                # Calculate power flow from generators and windfarms at this node
                power_flow_node = (
                        sum(windfarm_samples[wf][sample_n][h] for wf in WINDFARMS if
                            generator_nodes.get(wf) == node_i) +
                        gb.quicksum(
                            generator_production[(g, h)] for g in GENERATORS if generator_nodes.get(g) == node_i) -
                ...
                # Add line constraints for each line and time step
                model.addConstr(
                    -limit <= B[f"Bus{node_i}"][l] * power_flow_node,
                    name=f"Constraint_lb_line_({node1},{node2})_t={h}"
                )
                model.addConstr(
                    B[f"Bus{node_i}"][l] * power_flow_node <= limit,
